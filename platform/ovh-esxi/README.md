...
fleet of workers on a private `/24` behind NAT, sharing **one** public failover
IP. This is what makes the "$17/mo, 32 GB / 2 TB, a whole fleet on one dedicated
box" pitch real, and it doubles as the **test harness** for exercising the
mini-k8s as a whole on a single box. It is genuinely optional — it exists
because OVH is IP-stingy — and may eventually move out of vz entirely (e.g.
reused as a general homelab controller, or extended with a WireGuard VPN into
the private net).
...
- **`seed.yml -e vm_role=gateway`** — the one *static* seed. The gateway *is* the
  ...
  OUI). It boots before any worker. `int`'s address comes from `int_net` in
  `config.yml` — `genconfig.yml` picks it at random per deployment (see §5) so
  that different ESXi hosts land on different /24s.
- **`ansible/gateway.yaml`** (role `ansible/roles/gateway`) — IPv4 forwarding +
  **firewalld** NAT (external-zone masquerade + an internal→external forwarding
  ...
  ansible/requirements.yml`. The DHCP subnet (`gateway_int_net`) is **auto-detected**
  from `int`'s live IPv4 address — it is not a second copy of `int_net` that has
  to be hand-kept in sync with `config.yml`. This is also what makes it safe to
  wire multiple ESXi hosts' Internal segments together later (VPN/VPC between
  sites): each site's gateway just follows whatever /24 its own `int` already
  has, so distinct per-site subnets (from `genconfig.yml`'s randomization) never
  collide and never need manual reconciliation. Override with
  `-e gateway_int_net=10.20.30` only to force a DHCP subnet different from
  `int`'s actual address.
...
lives with the ansible `gateway` role, not here — it auto-detects the subnet
from `int`'s live address (which this `int_net` set), so there's nothing to
keep in sync by hand. `inventory.ini` is just the ESXi
