...
(The legacy shell seeder `make-rocky-vm.sh` mirrors these as `NAME` / `-g` /
`-n -g` / `-d`; it ships as a fallback until the Ansible seeder is verified
end-to-end.)
...
>
> **Legacy fallback:** the shell seeder (`make-rocky-vm.sh` + `config.env`) still
> ships while the Ansible seeder is being verified end-to-end. Its flow was
> `cp config.env.example config.env && vi config.env` then
> `gwid=$(./make-rocky-vm.sh -n -g gateway)`.
...
- **`seed.yml -e vm_role=gateway`** (legacy: `make-rocky-vm.sh -g`) — the one
  *static* seed. The gateway *is* the DHCP server, so it can't lease its own
...
| `make-rocky-vm.sh` + `config.env.example` | **Legacy** shell seeder (fallback until the Ansible seeder is verified end-to-end): `-g` gateway, `-n` no-boot, `-d` destroy | any Linux that can SSH to ESXi |
